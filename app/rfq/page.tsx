...
export default function RfqPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    const { data, error } = await s.rpc("create_rfq_atomic", {
      p_company_name: company.trim(), p_full_name: name.trim(), p_whatsapp: normalizedWa,
      p_city: city.trim(), p_industry: industry.trim(), p_email: email.trim() || null,
      p_part_id: partId.trim(), p_quantity: Number(qty),
    });
    if (error) { setStatus(error.message); return; }

    const result = data as { rfq_number: string };
    const waMessage = [
      `RFQ ${result.rfq_number}`, `Perusahaan: ${company.trim()}`, `PIC: ${name.trim()}`,
      `WhatsApp: ${whatsapp.trim()}`, `Kota: ${city.trim()}`, `Industri: ${industry.trim()}`,
      `Part ID: ${partId.trim()}`, `Qty: ${qty}`,
    ].join("\n");
  ...
  return (
    <main className="shell narrow">
      <section className="card">
        <span className="eyebrow">RFQ</span><h1>Request a Quote</h1>
        ...
        <form className="form" onSubmit={submit}>
          ...
          <label>Part ID<input value={partId} onChange={e => setPartId(e.target.value)} required />{partLabel && <small>{partLabel}</small>}</label>
